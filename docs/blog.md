...
2. **目录扫描**：默认扫描项目中的 `Resources` 目录，自动识别所有资源文件
3. **MSBuild 深度集成**：无需额外配置，安装 NuGet 包即可自动启用
4. **简洁的运行时 API**：提供异步和同步两种方式读取资源
5. **强类型访问**：自动生成类似 Android R 类的强类型访问代码，支持智能提示和编译时检查
6. **高度可配置**：通过 MSBuild 属性灵活配置资源目录、输出文件名等
