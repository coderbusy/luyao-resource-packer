...
```csharp
// 读取配置文件
string appConfig = await AppDataPackage.ReadAppConfigAsyncAsString();
string dbConfig = await AppDataPackage.ReadDbConfigAsyncAsString();
```
...
```csharp
// 读取 HTML 模板
string emailTemplate = await TemplateDataPackage.ReadEmailTemplateAsyncAsString();
string reportTemplate = await TemplateDataPackage.ReadReportTemplateAsyncAsString();
```
...
```csharp
// 读取不同语言的资源
string zhCN = await I18nDataPackage.ReadZhCNAsyncAsString();
string enUS = await I18nDataPackage.ReadEnUSAsyncAsString();
```
...
源代码生成器会自动创建一个静态类 `{AssemblyName}DataPackage`，提供强类型访问：

```csharp
using MyProject;  // 引入生成的类所在命名空间

// 使用常量访问资源键
Console.WriteLine(MyProjectDataPackage.Keys.config);
Console.WriteLine(MyProjectDataPackage.Keys.template);

// 读取资源为字符串
string config = await MyProjectDataPackage.ReadConfigAsyncAsString();
string template = await MyProjectDataPackage.ReadTemplateAsyncAsString();

// 读取资源为字节数组
byte[] data = await MyProjectDataPackage.ReadDataAsync();
```
...
namespace MyProject
{
    ...
    /// 自动生成的资源包访问类
    /// </summary>
    public static class MyProjectDataPackage
    {
...
```csharp
// 读取文档模板
var apiTemplate = await DocGenDataPackage.ReadApiTemplateAsyncAsString();
var indexTemplate = await DocGenDataPackage.ReadIndexTemplateAsyncAsString();
...
{
    "Development" => await AppDataPackage.ReadDevAsyncAsString(),
    "Staging" => await AppDataPackage.ReadStagingAsyncAsString(),
    "Production" => await AppDataPackage.ReadProdAsyncAsString(),
...
var levelData = await GameDataPackage.ReadLevel1AsyncAsString();
